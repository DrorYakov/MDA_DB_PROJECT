...
בחלק זה מפורטות טבלאות מסד הנתונים של מערכת מד"א, כולל פירוט השדות, אילוצי המערכת, והקשר הישיר של כל טבלה למסכי המערכת (GUI) השונים.

### 1. טבלת סוגי אירועים (`INCIDENT_TYPES`)
מרכזת את קטלוג סוגי מקרי החירום האפשריים במערכת והפרוטוקולים המקושרים אליהם.
* 🖥️ **קשר למסכי המערכת:**
  * **מסך מוקדן:** מופיע כרשימה נפתחת (Dropdown) בעת פתיחת קריאה, קובע אוטומטית את הדחיפות (צבע ההתרעה במסך).
  * **מסך טאבלט אמבולנס:** מקפיץ לצוות הרפואי חלון קופץ (Pop-up) עם הנחיות הפרוטוקול הרלוונטיות לאירוע.
* 📋 **פירוט שדות:**
  * **`Type_ID_`** (INT): מזהה ייחודי לסוג האירוע (PK).
  * **`Type_Name_`** (VARCHAR): שם סוג האירוע (למשל: "תאונת דרכים", "דום לב").
  * **`Default_Priority_`** (INT): רמת העדיפות/דחיפות המוגדרת כברירת מחדל לאירוע.
  * **`Protocol_Instructions_`** (VARCHAR): הנחיות הפרוטוקול לטיפול רפואי או תדרוך טלפוני.

### 2. טבלת מדווחים/מתקשרים (`CALLERS`)
שומרת את פרטי האנשים שהתקשרו למוקד החירום לדווח על אירוע.
* 🖥️ **קשר למסכי המערכת:**
  * **מערכת הטלפוניה במוקד:** בעת שיחה נכנסת, המערכת מזהה את מספר הטלפון ומקפיצה למוקדן את פרטי המתקשר (שם ושפה מועדפת), יחד עם היסטוריית קריאות קודמות.
* 📋 **פירוט שדות:**
  * **`Caller_ID_`** (INT): מזהה ייחודי של המתקשר (PK).
  * **`Phone_Number_`** (VARCHAR): מספר הטלפון של המתקשר (UNIQUE).
  * **`Full_Name_`** (VARCHAR): שמו המלא של המתקשר.
  * **`Language_Preference_`** (VARCHAR): שפת הדיבור המועדפת (ברירת מחדל 'Hebrew').

### 3. טבלת מטופלים (`PATIENTS`)
מכילה את המידע האישי והרפואי של המטופלים.
* 🖥️ **קשר למסכי המערכת:**
  * **מסך טאבלט אמבולנס:** טופס דיגיטלי לצוות בשטח, להזנת ת"ז ומשיכת היסטוריה רפואית.
  * **מסך קבלה במיון (בית חולים):** מציג לאחות הממיינת את פרטי החולה, הערות רפואיות ואלרגיות לקראת הגעתו.
* 📋 **פירוט שדות:**
  * **`Patient_ID_`** (INT): מזהה ייחודי של המטופל, ת"ז (PK).
  * **`First_Name_`** / **`Last_Name_`** (VARCHAR): שם פרטי ושם משפחה.
  * **`Birth_Date_`** (DATE): תאריך הלידה (כולל אילוץ למניעת תאריך עתידי `CHK_Patient_BirthDate`).
  * **`Gender_`** (VARCHAR): מין המטופל (אילוץ לערכים: Male, Female, Other, Unknown).
  * **`Medical_Notes_`** (VARCHAR): הערות רפואיות ומחלות רקע.
  * **`Insurance_Provider_`** (VARCHAR): קופת החולים או ספק הביטוח הרפואי.

### 4. טבלת בתי חולים (`HOSPITALS`)
רשימת בתי החולים הזמינים לפינוי וסטטוס העומס שלהם.
* 🖥️ **קשר למסכי המערכת:**
  * **מסך משגר (Dispatcher):** בתי חולים בסטטוס "Full" מופיעים באדום וחסומים לפינוי, ובתי חולים פנויים מופיעים בירוק.
  * **מסך מנהל מיון:** ממשק לעדכון מהיר של סטטוס העומס הנוכחי בחדר המיון.
* 📋 **פירוט שדות:**
  * **`Hospital_ID_`** (INT): מזהה ייחודי של בית החולים (PK).
  * **`Hospital_Name_`** (VARCHAR): שם בית החולים.
  * **`City_`** (VARCHAR): העיר בה ממוקם בית החולים.
  * **`Specialty_Unit_`** (VARCHAR): יחידות התמחות (למשל: "מרכז טראומה").
  * **`Current_Capacity_Status_`** (VARCHAR): סטטוס תפוסה (מוגבל ל: Low, Normal, High, Full, Diverting).

### 5. טבלת אירועים (`INCIDENTS`)
הטבלה המרכזית המתעדת את פתיחת הקריאות במוקד ואת הסטטוס שלהן.
* 🖥️ **קשר למסכי המערכת:**
  * **מסך "מבט על" למשגר (Dashboard):** רשימה דינמית של כל האירועים הפתוחים. אירועים בסטטוס "Pending" מהבהבים.
  * **מסך מוקדן:** תהליך יצירת הקריאה, תיעוד זמנים, וקביעת רמת חומרה.
* 📋 **פירוט שדות:**
  * **`Incident_ID_`** (INT): מזהה ייחודי של האירוע (PK).
  * **`Call_Start_Timestamp_`** / **`Call_End_Timestamp_`** (DATE): זמני שיחה (אילוץ: סיום מאוחר מהתחלה).
  * **`Severity_Level_`** (INT): רמת החומרה של האירוע (מוגבל באילוץ 1-5).
  * **`Status_`** (VARCHAR): סטטוס אירוע (Pending, Dispatched, On Scene, Transporting, Resolved, Cancelled).
  * **`Type_ID_`** (INT): סוג אירוע (FK ל-`INCIDENT_TYPES`).
  * **`Caller_ID_`** (INT): מזהה המתקשר (FK ל-`CALLERS`).

### 6. טבלת מיקומים (`LOCATIONS`)
המידע הגיאוגרפי והכתובת המדויקת של כל אירוע.
* 🖥️ **קשר למסכי המערכת:**
  * **מסך מפת GIS (מוקד):** מתרגם קואורדינטות לסיכות (Pins) על גבי מפה בזמן אמת.
  * **מערכת ניווט (GPS) באמבולנס:** שידור הכתובת המדויקת כולל קוד גישה לחסכון בזמן.
* 📋 **פירוט שדות:**
  * **`Location_ID_`** (INT): מזהה ייחודי (PK).
  * **`City_`** / **`Street_`** (VARCHAR): עיר ורחוב.
  * **`House_Num_`** / **`Floor_Num_`** (INT): בית וקומה (אילוץ: מספר בית גדול מאפס).
  * **`Entry_Code_`** (VARCHAR): קוד כניסה לבניין.
  * **`Latitude_`** / **`Longitude_`** (NUMERIC): קואורדינטות GPS (אילוץ תקינות).
  * **`Incident_ID_`** (INT): האירוע אליו שייך המיקום (FK ל-`INCIDENTS`).

### 7. טבלת שיגורי חירום (`EMERGENCY_DISPATCHES`)
מתעדת שילוח אמבולנסים ורכבי הצלה לזירה.
* 🖥️ **קשר למסכי המערכת:**
  * **מנהל צי רכבים:** צפייה במיקום ובסטטוס של כל רכב.
  * **אפליקציית כוננים:** כפתורי עדכון מהירים לסטטוס הצוות: "יצאנו", "הגענו", "עזבנו זירה".
* 📋 **פירוט שדות:**
  * **`Dispatch_ID_`** (INT): מזהה ייחודי לשיגור (PK).
  * **`Vehicle_ID_`** (VARCHAR): מזהה הרכב המשוגר.
  * **`Dispatch_Time_`**, **`Arrival_Time_`**, **`Departure_Time_`** (DATE): חותמות זמן היררכיות (כולל אילוץ רצף כרונולוגי `CHK_Dispatch_Times`).
  * **`Incident_ID_`** (INT): האירוע המשויך (FK ל-`INCIDENTS`).

### 8. טבלת מדדים רפואיים (`MEDICAL_MEASUREMENTS`)
תיעוד המדדים החיוניים של המטופלים.
* 🖥️ **קשר למסכי המערכת:**
  * **מוניטור באמבולנס:** הצגת גרפים. חיווי אדום מהבהב בערכי קיצון.
  * **מסך התרעות מיון:** צפייה מרחוק במדדי החולה המפונה להכנת צוות טראומה מראש.
* 📋 **פירוט שדות:**
  * **`Measurement_ID_`** (INT): מזהה בדיקה (PK).
  * **`Recorded_At_`** (DATE): מועד הבדיקה.
  * **`Systolic_BP_`** / **`Diastolic_BP_`** (INT): לחץ דם סיסטולי/דיאסטולי (אילוץ לוגי `CHK_Blood_Pressure`).
  * **`Pulse_`** (INT): דופק לב (אילוץ: 0-300).
  * **`Oxygen_Saturation_`** (INT): סטורציה (אילוץ: 0-100 `CHK_Oxygen_Sat`).
  * **`Dispatch_ID_`** / **`Patient_ID_`** (INT): מפתחות זרים לשיגור ולמטופל.

### 9. טבלת פרוצדורות שבוצעו (`PROCEDURES_PERFORMED`)
תיעוד טיפולים והליכים רפואיים שבוצעו בשטח.
* 🖥️ **קשר למסכי המערכת:**
  * **טופס רפואי:** צ'ק-ליסט דיגיטלי לפרמדיק לסימון פעולות ומידת הצלחתן.
  * **מסך בקרת איכות (QA):** דוחות סטטיסטיים למנהל הרפואי על אחוזי ההצלחה של פרוצדורות שונות.
* 📋 **פירוט שדות:**
  * **`Action_ID_`** (INT): מזהה פעולה (PK).
  * **`Procedure_Name_`** (VARCHAR): שם ההליך הרפואי.
  * **`Performed_At_`** (DATE): זמן ביצוע.
  * **`Success_Rate_`** (VARCHAR): רמת ההצלחה (אילוץ לערכים מוגדרים מראש: High, Medium, Low, Failed).
  * **`Dispatch_ID_`** / **`Patient_ID_`** (INT): מפתחות זרים לשיגור ולמטופל.

### 10. טבלת סיכומי העברה/פינוי (`TRANSFER_SUMMARIES`)
תהליך מסירת המטופל לצוות המיון בבית החולים.
* 🖥️ **קשר למסכי המערכת:**
  * **מסך חפיפה (Handover Screen):** טופס דיגיטלי לאישור העברת אחריות. הרופא המקבל חותם לאחר קריאת הערות הפרמדיק, סגירת מעגל הטיפול במערכת מד"א.
* 📋 **פירוט שדות:**
  * **`Transfer_ID_`** (INT): מזהה ייחודי להעברה (PK).
  * **`Receiving_Physician_`** (VARCHAR): שם הרופא הקולט בבית החולים.
  * **`Handover_Notes_`** (VARCHAR): סיכום והערות הצוות המפנה.
  * **`Arrival_At_Hospital_Time_`** (DATE): מועד ההגעה למיון.
  * **`Dispatch_ID_`** / **`Hospital_ID_`** (INT): מפתחות זרים לשיגור המקורי ולבית החולים הנקלט.
